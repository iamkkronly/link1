...
def scrape_mymp4movies(url):
    logger.info(f"Scraping Mymp4movies URL: {url}")
    ...
    try:
        ...
        # Check for Cloudflare or other protections if not using cloudscraper or if it failed
        if response.status_code in [403, 503]:
            logger.error(f"Access denied (Status: {response.status_code}). Site might be protected by Cloudflare.")
            print("Tip: If you are seeing 403/503 errors, you might need a Cloudflare solver like 'cloudscraper'.")
            return []
        ...
        # Strategy: Find all links and filter by quality text
...
if __name__ == "__main__":
    print("Mymp4movies Scraper")
    if len(sys.argv) > 1:
        url = sys.argv[1]
    else:
        url = input("Enter Mymp4movies Movie URL: ")
