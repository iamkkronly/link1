...
try:
    ...
    from gplinks_scraper import GPLinksScraper
    PLAYWRIGHT_AVAILABLE = True
except ImportError:
    logging.warning("Playwright not installed. OxxFile, MediaCM, and GPLinks scrapers will fail.")
...
def get_user_stats(user_id):
    """Helper to get or initialize user stats, resetting if the day has changed."""
    today = datetime.now().strftime('%Y-%m-%d')
    
    if user_id not in USER_STATS:
        USER_STATS[user_id] = {'date': today, 'scrape_count': 0, 'search_count': 0}
    else:
...
def scrape_watchadsontape(url):
    ...
    try:
        ...
        if match:
            ...
            return f"✅ <b>WatchAdsOnTape Bypassed!</b>\n\n🔗 <a href='{final_link}'>Download Link</a>"
        ...
        logging.error("Could not find download link pattern in WatchAdsOnTape page.")
        return "❌ Failed to scrape WatchAdsOnTape link."
...
def get_download_links(url):
    ...
    if is_gplinks_url(url):
        if not PLAYWRIGHT_AVAILABLE:
            return "❌ GPLinks bypass requires Playwright."
        try:
...
# --- ASYNC HELPER FOR SCRAPING WITH PROGRESS ---
async def perform_scrape_and_reply(url, update, context):
    ...
    try:
        # Helper to generate bar string
        def get_bar(percent):
            filled = int(percent / 10)
            bar = "▰" * filled + "▱" * (10 - filled)
            return f"Bypassing :- {percent}%「 {bar} 」"

        # Shared flag for the background task
        processing = True
        
        async def update_progress():
            percent = 0
            while processing and percent < 90:
                # Increment percent slightly
                ...
                try:
                    await context.bot.edit_message_text(
                        ...
                        text=get_bar(percent)
                    )
                except Exception:
                    pass # Ignore errors (like message not modified or rate limits)
                
                # Sleep to avoid hitting rate limits (0.5s is fast but safer than 5ms)
                await asyncio.sleep(0.5)

        # Start progress loop in background
        progress_task = asyncio.create_task(update_progress())

        # Run actual scrape synchronously in executor
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_html(
        ...
        "5. <b>/usage:</b> Check your daily stats."
    )
...
async def usage_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    used = stats['scrape_count']
    remaining = max(0, DAILY_LIMIT - used)
    searches = stats['search_count']
    
    msg = (
        f"📊 <b>Your Daily Usage Stats</b>\n\n"
        f"🔗 <b>Direct Link bypasses:</b> {used}/{DAILY_LIMIT}\n"
        f"✅ <b>Remaining:</b> {remaining}\n"
        f"🔎 <b>Search Downloads:</b> Unlimited (Doesn't count towards limit)\n"
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 1. URL HANDLING (DIRECT PASTE - COUNTED)
    if user_text.startswith("http"):
        # Check Limit for Scraping
        if stats['scrape_count'] >= DAILY_LIMIT:
            await update.message.reply_html(
                f"⚠️ <b>Daily Limit Reached!</b>\n\nYou have used {stats['scrape_count']}/{DAILY_LIMIT} direct link bypasses today.\n"
                "<b>Tip:</b> Searching for movies and downloading from search results is <b>Unlimited!</b>"
            )
            return
    ...
    # 2. SEARCH HANDLING
    # Increment search count (No limit applied here)
    ...
    try:
        ...
        # --- ADD EXTRA BUTTON FOR FILE STREAM BOT ---
...
async def handle_button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # NOTE: We do NOT increment the usage limit here. 
    # Search results are unlimited.
    
    await query.edit_message_text(f"✅ Selected. Processing... (Unlimited)")
